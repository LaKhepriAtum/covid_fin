...
def patientsum(request):
    ...
    df = df[["DATETIME" ,"GANGNAM" ,"GANGDONG","GANGBUK","GANGSEO"
            ,"GWANAK","GWANGJIN" ,"GURO","GEUMCHEON","NOWON"
            ,"DOBONG","DDM","DONGJAK","MAPO","SDM","SEOCHO"
            ,"SEONGDONG","SEONGBUK","SONGPA","YANGCHEON"
            ,"YDP","YONGSAN","EP","JONGNO","JUNGGU","JUNGNANG"]]
    ...
    for x in df.to_records(index=False):
        row=[  x[0] ,int(x[1] ),int(x[2] ),int(x[3] ),int(x[4] ),int(x[5] ),int(x[6] ),int(x[7] ),int(x[8] ),int(x[9] )
            ...
            ,int(x[20] ),int(x[21] ),int(x[22] ),int(x[23] ),int(x[24] ),int(x[25])]
        # 전체 list에 담음.
        rows.append(row)
...
def patientadd(request):
    ...
    df = df[["DATETIME" ,"GANGNAMADD" ,"GANGDONGADD","GANGBUKADD","GANGSEOADD"
            ,"GWANAKADD","GWANGJINADD" ,"GUROADD","GEUMCHEONADD","NOWONADD"
            ,"DOBONGADD","DDMADD","DONGJAKADD","MAPOADD","SDMADD","SEOCHOADD"
            ,"SEONGDONGADD","SEONGBUKADD","SONGPAADD","YANGCHEONADD"
            ,"YDPADD","YONGSANADD","EPADD","JONGNOADD","JUNGGUADD","JUNGNANGADD"]]
...
def death(request):
    ...
    params ={'serviceKey' : serial_key, 'pageNo' : '1', 'numOfRows' : '10000', 'startCreateDt' : days, 'endCreateDt' : days}
    ...
    for x in df.to_records(index=False):
        ...
        sd = Death(date= date , location= location, deathcnt=deathcnt, incdec= incdec ,defCnt=defCnt)  
        sd.save()
